...
namespace SecretsOfTheScug.Skills.SkillDefTypes
{
    class DualSkillDef : SkillDef
	{
		...
		public override EntityState InstantiateNextState([NotNull] GenericSkill skillSlot)
		{
			...
			if (instanceData.isPlayerControlled)
			{
				...
				entityState = EntityStateCatalog.InstantiateState(ref state);
			}
            else
			{
				SerializableEntityStateType stateAlt = this.alternateActivationState;
				entityState = EntityStateCatalog.InstantiateState(ref stateAlt);
			}
